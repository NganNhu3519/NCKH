...
x=x';
n = length(x);
%numsize = sqrt(length(x));
figure,plot(x)
%___MEASUREMENT MATRIX___
m = 20; % NOTE: small error still present after increasing m to 1500;
Phi = randn(m,n);
...
% subplot(1,3,1), imagesc(reshape(x,numsize,numsize)), xlabel('original'), axis image
% subplot(1,3,2), imagesc(reshape(x2,numsize,numsize)), xlabel('least squares'), axis image
% subplot(1,3,3), imagesc(reshape(x1,numsize,numsize)), xlabel('basis pursuit'), axis image
% colormap gray
figure('name','Compressive sensing image reconstructions')
subplot(3,1,1),plot(x), xlabel('original'), axis image
subplot(3,1,2),plot(x2), xlabel('least squares'), axis image
subplot(3,1,3), plot(x1), xlabel('basis pursuit'), axis image
colormap gray
