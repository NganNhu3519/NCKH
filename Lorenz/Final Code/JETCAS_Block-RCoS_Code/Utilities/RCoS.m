function [U, out] = RCoS(A,b,p,q,opts)
...
[U,mu,beta] = CoSAR_init(p,q,Atb,scl,opts);    % U: p*q
if mu > muf
    mu = muf;
end
if beta > betaf
...
for ii = 1:opts.maxit
    ...
    if Nonlocal_flag
        Para = [];
        X = Nonlocal_Solver(reshape((U(:)-gamma/theta)/scl,p,q), opts.lambda, Para);
...
function [U,mu,beta] = CoSAR_init(p,q,Atb,scl,opts)
